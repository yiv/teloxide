        ...
        N: AsRef<str>,
{
    ...
    // parse_buff_command(text)
}

// pub fn parse_buff_command<'a>(
//     text: &'a str,
...
//             ).unwrap();
//     }
//     let h: Vec<&str> = HASHTAG_REGEX.find_iter(text).map(|mat| mat.as_str().replace("${/", "").replace("}", "").as_str()).collect();
//     if h.len() > 0 {
//         let command = h[0].clone();
//         Some((command, vec![]))
//     } else {
//         None
//     }
